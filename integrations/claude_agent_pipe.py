...
class Pipe:
    class Valves(BaseModel):
        ...
        WORKDIR_ROOT: str = Field(
            default=str(Path(tempfile.gettempdir()) / "claude-agent-pipe"),
    ...
    def pipes(self) -> List[Dict[str, str]]:
        # Expose one entry per model so all appear in the model picker. The
        # selected pipe's id (a real Claude model ID) is read back in pipe()
        # via body["model"] and passed to the SDK, overriding the MODEL valve.
        return [
            {"id": "claude-opus-4-7", "name": "Claude Code (Opus 4.7)"},
            {"id": "claude-sonnet-4-6", "name": "Claude Code (Sonnet 4.6)"},
            {"id": "claude-haiku-4-5", "name": "Claude Code (Haiku 4.5)"},
        ]
    ...
    async def pipe(
        ...
        # (see pipes()). Fall back to the MODEL valve if parsing yields nothing.
        selected_model = self.valves.MODEL
        raw_model = str(body.get("model", "")) if isinstance(body, dict) else ""
        if raw_model:
